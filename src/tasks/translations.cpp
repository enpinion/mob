...
translations::projects::lang translations::projects::create_lang(
	const std::string& project_name, const fs::path& main_ts_file)
{
...
void translations::do_clean(clean c)
{
	...
	{
		op::delete_file_glob_recurse(cx(),
			conf().path().install_extensions(), "*.qm",
			op::optional);
	}
}
...
void translations::do_build_and_install()
{
	...
	for (auto& p : ps.get())
	{
		if (!project_to_extension.contains(p.name)) {
			cx().warning(context::generic, "found project {} but no matching extension", p.name);
			continue;
		}
		...
		if (!fs::exists(base)) {
			cx().warning(context::generic, "found project {} for extension {} extension is not built", p.name, project_to_extension[p.name]);
			continue;
		}
